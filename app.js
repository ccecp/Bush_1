...
function openModule(i){current=i;let m=modules[i];document.getElementById("moduleCode").textContent="SECȚIUNEA "+m.id;document.getElementById("moduleTitle").textContent=m.title;const ap=document.getElementById("audioPlayer");ap.pause();ap.removeAttribute("src");ap.load();if(m.audio){ap.src=encodeURI(m.audio)+"?v=20260919-11";ap.load();}const note=document.getElementById("audioStatus");if(note)note.textContent=m.audio?"":"Audio indisponibil pentru această secțiune.";showTab("audio");renderQuiz();buildMenu();window.scrollTo({top:150,behavior:"smooth"})}
...
(function(){
 ...
 ap.addEventListener("error",function(){
   const note=document.getElementById("audioStatus");
   if(note)note.textContent="Fișierul audio nu s-a încărcat. Încearcă reîncărcarea paginii.";
 });
